...
@section('content')
    ...
    @endif
    <form method="POST" action="{{route('category.postCreate')}}" accept-charset="UTF-8"  class="form-horizontal form-label-left" id="main-form">
        @csrf
        <div class="form-group">
            <label for="name" class="control-label col-md-3 col-sm-3 col-xs-12">Name</label>
            <div class="col-md-6 col-sm-6 col-xs-12">
                <input class="form-control col-md-6 col-xs-12" name="name" type="text"  id="name" value="{{old('name')}}">
                @error('name')
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
                <select class="form-control col-md-6 col-xs-12" id="parent_id" name="parent_id">
                    ...
                    @foreach($data as $item)
                            <option value="{{$item['id']}}" {{old('parent_id') == $item['id'] ? 'selected' : false}} >{{$item['name']}}</option>
                    @endforeach
